...
contract Merge is ERC721, ERC721Metadata {
    ...
    function ensureValidClass(uint256 class2) private pure {
        require(MIN_CLASS_INCL <= class2 && class2 <= MAX_CLASS_INCL, "Merge: Class must be [1, 4].");
    }
     ...
     function mint(uint256[] calldata values_) external onlyValidSender {
        ...
        while (valueIx < values_.length) {            
            ...
            if (isSentinelMass(values_[valueIx])) {
                ...
                (/* uint256 class2 */, uint256 mass) = decodeClassAndMass(values_[valueIx]);
    ...
    function encodeClassAndMass(uint256 class2, uint256 mass) public pure returns (uint256) {
        ensureValidClass(class2);
        ensureValidMass(mass);
        return ((class2 * CLASS_MULTIPLIER) + mass);
    }

    function decodeClassAndMass(uint256 value) public pure returns (uint256, uint256) {
        uint256 class2 = decodeClass(value);
        uint256 mass = decodeMass(value);
        return (class2, mass);
    }

    function decodeClass(uint256 value) public pure returns (uint256 class2) {
        class2 = value / CLASS_MULTIPLIER; // integer division is ‘checked’ in Solidity 0.8.x
        ensureValidClass(class2);
    }    
